...
/* harmony default export */ var SparePartvue_type_script_lang_js_ = ({
  ...
  methods: (_methods = {
    ...
    loadDefaults: function loadDefaults() {
      ...
      debugger; // axios.get(`/api/admin-panel/devices/getAlldevices?page=${this.page}`)
      //         .then(res => {
      //           debugger
      //             if (!this.$roleEquals('admin') || this.$roleEquals('system_manager')){
      //                 this.devices = res.data.data.filter(item => item.id == this.department);
      //                 console.log('datrata',this.devices);
  ...
  }), _defineProperty(_methods, "computed", {
    ...
    axios_default.a.get("/api/admin-panel/devices/getAlldevices?page=".concat(this.page)).then(function (res) {
      ...
      if (!_this5.$roleEquals('admin') || _this5.$roleEquals('system_manager')) {
        _this5.devices = res.data.data.filter(function (item) {
          return item.id == _this5.department;
        });
        console.log('datrata', _this5.devices);
        _this5.types = _this5.types.filter(function (item) {
          return item.id == _this5.department;
        });
      } else {
        _this5.devices = res.data.data;
      }
    })["catch"](function (err) {
      console.warn(err);
    });
