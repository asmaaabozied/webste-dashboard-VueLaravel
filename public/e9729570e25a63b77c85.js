  ...
  methods: (_methods = {
    ...
    loadDefaults: function loadDefaults() {
      ...
      axios_default.a.get("/api/admin-panel/devices/getAlldevices?page=".concat(this.page)).then(function (res) {
        debugger;
        _this3.devices = res.data.data.resource.datafilter(function (item) {
          return item.id == _this3.department;
        });
        console.log('datrata', _this3.devices); // if (!this.$roleEquals('admin') || this.$roleEquals('system_manager')){
