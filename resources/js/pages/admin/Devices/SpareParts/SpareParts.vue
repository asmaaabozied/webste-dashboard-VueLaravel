...
export default {
  ...
  methods: {
    open(id) {
        console.log(id);
      this.$router.push({name: "SpareParts", params: {id: id}});
    },
    addItem(id) {
      this.$router.push({name: "SpareParts", params: {id: 'create'}});
    },
    deleteItem(id) {
    ...
    load() {
      axios
       ...
       .get(`/api/admin-panel/sparePart/getAlldevices?page=${this.page}`)
    ...
    confirmDeleteItem() {
      axios
          .delete(`/api/admin-panel/sparePart/delete/${this.deleteItemId}`)
          .then((res) => {
            this.$notify({
